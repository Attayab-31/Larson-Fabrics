...
import { FOOTER_QUICK_LINKS, SOCIAL_LINKS } from "@/lib/navigation";
...
export function Footer() {
  ...
  return (
    <footer
      ref={footerRef}
      className="bg-[#0A1F5C] text-white"
    ...
    >
      <div ref={innerRef} className="mx-auto max-w-7xl px-6 py-16 md:px-12 md:py-20 lg:px-20">
        <div className="grid gap-12 md:grid-cols-3 md:gap-8">
          ...
          <div>
            <Link href="/" className="inline-flex items-baseline gap-1">
              <span className="font-display text-3xl italic">Larson</span>
              <span className="font-body text-sm font-light uppercase tracking-[0.2em]">
                Fabrics
              </span>
            </Link>
          ...
          {/* Quick links */}
          <div>
            <p className="label-caps mb-6 text-gold">Quick Links</p>
          ...
          {/* Contact */}
          <div>
            <p className="label-caps mb-6 text-gold">Contact</p>
            <address className="space-y-3 font-body text-sm not-italic text-white/80">
              <p>{BRAND.address}</p>
              <p>
                <a href={`tel:${BRAND.phone.replace(/\s/g, "")}`} className="hover:text-gold">
                  {BRAND.phone}
                </a>
              </p>
              <p>
              ...
              <p className="pt-2 text-white/60">{BRAND.hours}</p>
            </address>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-between gap-4 font-body text-xs text-white/50 md:flex-row">
          ...
          <p className="uppercase tracking-widest">Azam Market · Lahore · Pakistan</p>
        </div>
      </div>
    </footer>
  );
